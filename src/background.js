  ...
  function (details) {
    try {
      if (details.requestBody && details.requestBody.formData) {
        ...
        //   console.log("User Id:", snapchatUserId);
  ...
  function (details) {
    try {
      if (details.requestHeaders) {
        ...
        if (authHeader) {
        //   console.log("User Authorization Key:", authHeader.value);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "getData") {
    ...
    chrome.storage.local.get(
      ["snapchatUserId", "user_authorization", "user_cookie"],
      (result) => {
        sendResponse({
...
async function getCookies() {
    try {
        ...
        if (cookies) {
            ...
            await chrome.storage.local.set({ user_cookie: cookieString });
            return cookies;
        } else {
    ...
    try {
        ...
        const userId = await new Promise(resolve => {
            chrome.storage.local.get(['snapchatUserId'], result => {
                resolve(result.snapchatUserId || snapchatUserId);
            });
        });
        
        const authToken = await new Promise(resolve => {
            chrome.storage.local.get(['user_authorization'], result => {
                resolve(result.user_authorization || authorization_key);
            });
        });
        ...
        const friendsList = await snapApi.getFriendsList(userId, authToken);
        
        if (friendsList) {
            await chrome.storage.local.set({ user_friends: friendsList });
        }
